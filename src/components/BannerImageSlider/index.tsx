...
const bannerImageSliderStyles = cva(
  ['flex flex-col max-w-[90%] lg:max-w-full mx-auto my-0 lg:w-[518px] gap-10'],
  {
    variants: {},
  },
);
...
const BannerImageSlider: StorefrontFunctionComponent<BannerImageSliderProps> = (
  props,
) => {
  ...
  return (
    <div className={cx(bannerImageSliderStyles())}>
      <div
        className="embla__viewport overflow-hidden max-w-full rounded-3xl"
        ref={emblaRef}
      >
        <div className="embla__container flex  lg:max-w-[520px] h-[450px]">
          {slides.map(({ image, link, alt }: any, index) => (
            <div className="embla__slide flex-shrink-0" key={index}>
              <Link to={link}>
                <img
                  className="embla__slide__number object-contain"
                  src={image}
                  alt={alt}
                />
              </Link>
          ...
          {scrollSnaps.map((_, index) => (
            <DotButton
              ...
              className={'embla__dot w-2.5 h-2.5  rounded-full bg-gray-200'.concat(
                index === selectedIndex
                  ? ' embla__dot--selected w-4 h-4 bg-orange-500'
                  : '',
              )}
            />
          ))}
